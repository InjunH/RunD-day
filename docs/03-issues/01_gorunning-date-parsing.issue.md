...
1. **열 매핑 수정** (`extractRaces()` 함수)
   - 기존: `[날짜, 이름, 주최, 장소, 상태]` (잘못된 가정)
   - 수정: `[순번, 대회명, 종목, 지역, 장소, 주최, 등록상태]` (실제 구조)

2. **인터페이스 수정** (`GoRunningRaceRow`)
   - `date` 필드 제거 (테이블에 날짜 열 없음)
   - `distance`, `region` 필드 추가

3. **날짜 추출 전략 변경**
   - 대회명에서 날짜 패턴 추출 (`extractDateFromName()`)
...
|------|--------|-------|--------|
| `2000-12-31` 날짜 | 다수 | **0개** | ✅ |
| `organizer`에 거리 정보 | 있음 | **없음** | ✅ |
| gorunning 이벤트 수 | 142개 | 142개 | ✅ |

### 8.3. 알려진 한계

– 대회명에 날짜 정보가 없는 경우 `2026-12-31` (연말) 기본값 사용
– 정확한 날짜를 위해서는 상세 페이지 크롤링 필요 (향후 개선 사항)
